...
private:
    ...
    // Current point cloud timestamp for consistent time across all functions
    builtin_interfaces::msg::Time currentCloudStamp;

public:
    TraversabilityMapping()
        : Node("traversability_mapping")
        , pubCount(1)
        , mapArrayCount(0)
    {
        // Initialize TF2
    ...
    void cloudHandler(const sensor_msgs::msg::PointCloud2::SharedPtr laserCloudMsg)
    {
        ...
        // Store current cloud timestamp
        currentCloudStamp = laserCloudMsg->header.stamp;
        // RCLCPP_INFO(this->get_logger(), "Received filtered cloud with %d points, stamp: %.6f",
    ...
    void updateCellOccupancy(mapCell_t* thisCell, PointType* point)
    {
        ...
        thisCell->last_updated_time = rclcpp::Time(currentCloudStamp);
    }
    ...
    bool findPointGridInMap(grid_t* gridOut, PointType* point)
    {
        ...
        if (thisGrid.cubeX >= 0 && thisGrid.cubeX < mapArrayLength && thisGrid.cubeY >= 0 && thisGrid.cubeY < mapArrayLength) {
    ...
    void publishLocalMap()
    {
        ...
        if (pubOccupancyMapLocal->get_subscription_count() == 0 && pubOccupancyMapLocalHeight->get_subscription_count() == 0)
            return;
    ...
    bool getRobotPosition(builtin_interfaces::msg::Time stamp)
    {
        try {
            ...
            //             rclcpp::Time(stamp).seconds(), ex.what());
            return false;
        }
