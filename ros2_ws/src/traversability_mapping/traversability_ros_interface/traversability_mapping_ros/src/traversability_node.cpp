...
private:
    ...
    void rotateOccupancyGrid180(nav_msgs::msg::OccupancyGrid &grid)
    {
        const uint32_t width = grid.info.width;
        const uint32_t height = grid.info.height;

        std::vector<int8_t> rotated(width * height);
        ...
        for (uint32_t y = 0; y < height; ++y)
        {
            for (uint32_t x = 0; x < width; ++x)
            {
                ...
                uint32_t dst_x = width - 1 - x;
                uint32_t dst_y = height - 1 - y;
                uint32_t dst_idx = dst_y * width + dst_x;

                rotated[dst_idx] = grid.data[src_idx];
            }
        }

        grid.data = std::move(rotated);
    }

    void rotateOccupancyGrid90(nav_msgs::msg::OccupancyGrid &grid)
    {
        const uint32_t width = grid.info.width;
        const uint32_t height = grid.info.height;

        const uint32_t new_width = height;
        const uint32_t new_height = width;

        std::vector<int8_t> rotated(new_width * new_height);
        ...
        for (uint32_t y = 0; y < height; ++y)
        {
            for (uint32_t x = 0; x < width; ++x)
            {
                ...
                // 90° CCW
                uint32_t dst_x = y;
                uint32_t dst_y = width - 1 - x;

                uint32_t dst_idx = dst_y * new_width + dst_x;
    ...
    void publishTraversabilityData()
    {
        ...
        if (localMap != nullptr)
        {
            ...
            {
                //rotateOccupancyGrid180(*gridmap);
                rotateOccupancyGrid90(*gridmap);
