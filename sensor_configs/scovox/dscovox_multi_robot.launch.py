...
_ARGS = {
    "map_frame": ("map_global", "Global/world frame shared by every robot (NDT vs the same gt_map)"),
    "use_sim_time": ("false", "Use /clock (true for bag replay)"),
    "deskew_mode": ("on", "auto|on|off — 'off' silences the IMU-not-ready note"),
    "min_range": ("1.0", "Per-scan min integration range (m)"),
    "max_range": ("20.0", "Per-scan max integration range (m) — RAISE to widen the footprint"),
    "share_roi_z_min": ("-0.5", "Vertical share-band lower bound (m, map frame); wired to mapper+merger"),
    "share_roi_z_max": ("2.0", "Vertical share-band upper bound (m). min>=max (0/0) = band OFF"),
...
def launch_setup(context, *args, **kwargs):
    ...
    nodes.append(Node(
        ...
        parameters=[{
            ...
            #"semantic_top_k": 2.0, # Must match the senders compile-time K_TOP 
        }],
    ))
    ...
    nodes.append(Node(
        ...
        name=f"map_global_to_map_curt",
        arguments=[
            "--x", "0.42", "--y", "-3.01", "--z", "0",
            "--qx", "0", "--qy", "0", "--qz", "0", "--qw", "1",
            "--frame-id", map_frame, "--child-frame-id", "map_curt",
        ],
    ))
    ...
    nodes.append(Node(
        package="tf2_ros",
        executable="static_transform_publisher",
        name=f"map_global_to_map",
        arguments=[
            "--x", "0", "--y", "0", "--z", "0",
            "--qx", "0", "--qy", "0", "--qz", "0", "--qw", "1",
            "--frame-id", map_frame, "--child-frame-id", "map",
        ],
    ))
